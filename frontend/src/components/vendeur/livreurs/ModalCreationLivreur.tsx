...
import { creerLivreur } from '../../../services/vendeur/vendeurLivreurService';
import type { FormulaireCreationLivreurVendeur } from '../../../types/vendeur/livreur';
...
type EtatFormulaire = FormulaireCreationLivreurVendeur;
type ErreurFormulaire = Partial<EtatFormulaire & { global: string }>;
...
/**
 * Modal de création d'un livreur par le vendeur.
 * Envoie un email d'invitation au livreur avec les identifiants temporaires.
 */
export default function ModalCreationLivreur({ ouvert, onFermer, onSucces }: Props) {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="modal-creation-livreur-vendeur-titre"
    >
      <div className="bg-surface w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 id="modal-creation-livreur-vendeur-titre" className="text-base font-bold text-primary">
                Ajouter un livreur
              </h2>
              <p className="text-xs text-[#74777d]">Un email d'invitation lui sera envoyé automatiquement.</p>
            </div>
          </div>
          <button
          ...
          <form onSubmit={handleSoumettre} noValidate className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col gap-1.5">
                <label htmlFor="livreur-prenom" className="label-admin">
                ...
                <input
                  ...
                  aria-describedby={erreurs.prenom ? 'livreur-prenom-error' : undefined}
                  className={champClass(!!erreurs.prenom)}
                />
                {erreurs.prenom && (
                  <p id="livreur-prenom-error" role="alert" className="flex items-center gap-1 text-xs text-red-600">
              ...
              <div className="flex flex-col gap-1.5">
                <label htmlFor="livreur-nom" className="label-admin">
                  <User size={12} className="inline mr-1" aria-hidden="true" />
                  Nom *
                </label>
                <input
                  ...
                  aria-describedby={erreurs.nom ? 'livreur-nom-error' : undefined}
                  className={champClass(!!erreurs.nom)}
                />
                {erreurs.nom && (
                  <p id="livreur-nom-error" role="alert" className="flex items-center gap-1 text-xs text-red-600">
            ...
            {/* Email */}
            <div className="flex flex-col gap-1.5">
              <label htmlFor="livreur-email" className="label-admin">
              ...
              <input
                ...
                aria-describedby={erreurs.email ? 'livreur-email-error' : undefined}
                className={champClass(!!erreurs.email)}
              />
              {erreurs.email && (
                <p id="livreur-email-error" role="alert" className="flex items-center gap-1 text-xs text-red-600">
