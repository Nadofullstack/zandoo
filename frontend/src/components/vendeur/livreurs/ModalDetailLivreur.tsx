...
import { getLivreurParId } from '../../../services/vendeur/vendeurLivreurService';
...
export default function ModalDetailLivreur({ livreurId, onFermer }: Props) {
  ...
  useEffect(() => {
    ...
    getLivreurParId(livreurId)
      .then((rep) => { if (!annule) setLivreur(rep.data.livreur); })
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="modal-detail-livreur-vendeur-titre"
    >
      <div className="bg-surface w-full max-w-2xl rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 id="modal-detail-livreur-vendeur-titre" className="text-base font-bold text-primary">
                Détails du livreur
              </h2>
              <p className="text-xs text-[#74777d]">Informations complètes du livreur.</p>
            </div>
          </div>
          <button
            onClick={onFermer}
                      ...
                      <li key={entree._id} className="flex items-start gap-3 text-sm">
                        ...
                        <div className="min-w-0">
                          <p className="text-[#74777d] text-xs">
                            ...
                            {entree.modifiePar && (
                              <> · par <span className="font-semibold text-primary">
                                {typeof entree.modifiePar === 'string'
                                  ? entree.modifiePar
                                  : entree.modifiePar.fullName}
                              </span></>
                            )}
                          </p>
                          {entree.raison && (
