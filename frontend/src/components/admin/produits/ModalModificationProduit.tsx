...
/* ── Types ────────────────────────────────────────────────────────────────── */
interface Props { produitId: string | null; onFermer: () => void; onSucces: () => void; }
...
export default function ModalModificationProduit({ produitId, onFermer, onSucces }: Props) {
  ...
  /* ── Variantes photos ────────────────────────────────── */
  const ajouterVariantePhoto   = () => setVariantesPhotos((p) => [...p, { nom: '', photos: [] }]);
  ...
  const ajouterPhotosVariante = (iv: number, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setVariantesPhotos((prev) => {
      const v = [...prev];
      v[iv] = { ...v[iv], photos: [...v[iv].photos, ...fichiers.map((f) => ({ fichier: f, preview: URL.createObjectURL(f), uploadee: false }))] };
      return v;
    });
    if (e.target) e.target.value = '';
  };
  const supprimerPhotoVariante = (iv: number, ip: number) => {
    ...
    setVariantesPhotos((prev) => {
      const v = [...prev]; v[iv] = { ...v[iv], photos: v[iv].photos.filter((_, k) => k !== ip) }; return v;
    });
  };
  ...
  /* ── Variantes produit ───────────────────────────────── */
  const ajouterVariante    = () => setForm((p) => ({ ...p, variantes: [...p.variantes, { nom: '', valeurs: [] }] }));
  ...
  const uploaderCouverture = async (): Promise<string | null> => {
    ...
    if (couverture.uploadee || !couverture.fichier) return couverture.preview;
  ...
  const handleSoumettre = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!videoPreview) urlVideo = undefined;
      ...
      setErreurs((p) => ({ ...p, global: err instanceof Error ? err.message : 'Erreur lors de la modification.' }));
      ...
      <div className="bg-surface w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 flex-shrink-0">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-blue-100 rounded-xl"><Pencil size={18} className="text-blue-600" aria-hidden /></div>
            <div>
              <h2 id="modal-modif-produit-titre" className="text-base font-bold text-primary">Modifier le produit</h2>
          ...
          {ONGLETS.map(({ id, libelle, icone: Icone }) => (
            <button key={id} type="button" onClick={() => setOnglet(id)}
              className={['relative flex items-center gap-1.5 px-4 py-3 text-xs font-semibold transition-colors',
                onglet === id ? 'text-accent border-b-2 border-accent' : 'text-[#74777d] hover:text-primary'].join(' ')}>
              <Icone size={13} aria-hidden />
              {libelle}
              {aErreurOnglet(id) && <span className="ml-1 w-2 h-2 rounded-full bg-red-500 inline-block" />}
            </button>
          ))}
        </div>
        ...
        <form onSubmit={handleSoumettre} noValidate className="flex flex-col flex-1 min-h-0">
          <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
            {chargProduit && (
              <div className="flex items-center gap-2 text-xs text-primary bg-gray-50 border border-gray-200 rounded-xl p-3">
                <RefreshCw size={13} className="animate-spin" /> Chargement du produit…
              </div>
            )}
            ...
            {uploadEnCours && (
              <div className="flex items-center gap-2 text-xs text-accent bg-orange-50 border border-orange-200 rounded-xl p-3">
                <RefreshCw size={13} className="animate-spin" /> Upload en cours…
              </div>
            )}
            ...
            {!chargProduit && (
              <>
                {onglet === 'infos' && (
                  <OngletInformations
                    form={form} erreurs={erreurs} categories={categories} chargCat={chargCat}
          ...
          <div className="flex items-center justify-between px-6 py-4 border-t border-gray-100 flex-shrink-0 bg-surface">
            ...
            <div className="flex gap-3">
              ...
              {onglet !== 'variantes' ? (
                <button type="button" onClick={() => setOnglet(ORDRE[ORDRE.indexOf(onglet) + 1])}
