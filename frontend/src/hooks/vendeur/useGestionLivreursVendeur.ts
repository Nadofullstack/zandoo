...
import type {
  ...
  StatistiquesLivreursVendeur,
} from '../../types/vendeur/livreur';
import type { Pagination } from '../../types/admin/common';
import {
  getMesLivreurs,
...
interface EtatHook {
  ...
  statistiques: StatistiquesLivreursVendeur | null;
  chargement: boolean;
...
export function useGestionLivreursVendeur(): EtatHook {
  const [livreurs, setLivreurs]                   = useState<LivreurVendeur[]>([]);
  const [pagination, setPagination]               = useState<Pagination | null>(null);
  const [statistiques, setStatistiques]           = useState<StatistiquesLivreursVendeur | null>(null);
  ...
  const changerStatut = useCallback(
    async (id: string, statut: StatutLivreurVendeur, raison?: string) => {
